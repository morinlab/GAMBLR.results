...
#'
#' @description Bring together all derived sample-level results from some GAMBL RNA pipelines.
#'
...
#' If a dataframe is not provided, the function will default to all mrna metadata returned with `get_gambl_metadata`. For more information on how to get the most out of this function,
#' refer to function examples, vignettes and parameter descriptions.
#'
#' @param these_samples_metadata Optional argument to use a user specified metadata df, must include seq_type column to specify desired seq types. If not provided, defaults to all mrna samples with `get_gambl_metadata`. 
...
#' @examples
#' \dontrun{
#' #get collated results for all mRNA samples, using cached results
#' mrna_metadata = get_gambl_metadata() %>% dplyr::filter(seq_type == "mrna")
#' mrna_collated_everything = collate_RNA_results(these_samples_metadata = mrna_metadata,
#'                                               from_cache = TRUE,
#'                                               write_to_file = FALSE)
#'
...
#'
#' fl_collated = collate_RNA_results(these_samples_metadata = fl_metadata,
#'                               write_to_file = FALSE,
#'                               from_cache = TRUE)
#' }
