...
#' within that data frame. This function relies on a set of specific internal functions [GAMBLR.results::id_ease] and [GAMBLR.results::get_manta_sv_by_samples] (if `from_cache = FALSE`).
...
#'
get_new_manta_sv = function(these_sample_ids = NULL,
  ...
  this_meta = id_ease(these_samples_metadata = these_samples_metadata,
                      ...
                      verbose = verbose) #only genome samples have manta results
  ...
  if(from_flatfile){
    if(from_cache){
      ...
      if(!file.exists(output_file)){
        warning(paste("File not found:", output_file))
        return(NULL)
      }
