...
If you used `renv` to install the dependencies, you will need to add these parameters to your command

```bash
Rscript tools/logExampleOutputs.R --renv TRUE --path /full/path/to/renv
```

E.g. if your lockfile is at `/projects/scratch/username/GAMBLR-dev/renv.lock` the `--path` argument would be `/projects/scratch/username/GAMBLR-dev`.
